...
class AppEngineConnection(object):
    ...
    def add_new_user(self):
        ...
        password = hashlib.sha1(raw_input("password: ")).hexdigest()
        post_values['salt'] = generate_salt()
        post_values['password'] = hashlib.sha1(password + post_values['salt']).hexdigest()
        post_values['timezone'] = raw_input("timezone: ")
        
        encoded_vals = urllib.urlencode(post_values)
        req = urllib2.urlopen(self.url + "/adduser", encoded_vals)
    ...
    def request_session_key(self):
        ...
        if response == "OK":
            ...
            self.expires = req.readline().strip()
            self.pickle_key()
            return True
        else:
            self.error = response
    ...
    def load_session_key(self):
        ...
        try:
            print "trying to load session key"
            ...
            print "Key doesnt exist, requesting"
            if not self.request_session_key():
                print "Key could not be retrieved, exiting"
                return False
    ...
    for arg in args:
        ...
        if arg == "password":
            if vals["password"] != "":
                ...
                vals["passreq"] = 0
    try:
        vals["password"] = hashlib.sha1(vals["password"] + vals["salt"]).hexdigest()
    except KeyError:
        pass
    return vals
...
def run():
    ...
    groups = conn.app_engine_request({}, "/usr/groups")[1]
    print "groups:", groups
    while True:
